FFBS.Norm <- function(U, eventtimes, obs, param){
n <- length(obs)
Uniformtimes <- U$U
JumpProb <- U$JumpProb
...
for (i in 2:(k+1)){
   ...
   ff <- logLik.Norm(eventtimes, obs, param, Uniformtimes[i-1], Uniformtimes[i])
...
theta[m+1] = rnorm(1, (muprior/sigmaprior^2 + N[m+1]*mean(obs[cp[m]:n])/Sigma^2)/precision[m+1], sd=1/precision[m+1])
cp <- cp[-1]
} else{
       cp <- NULL
       Q <- NULL
