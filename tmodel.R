...
mu[i] <- rnorm(1, temp2/temp1, Alpha^2/temp1)
for (j in cp[i] : (cp[i+1] - 1)){
...
mu[m+1] <- rnorm(1, temp2/temp1, Alpha^2/temp1)

for (j in cp[m+1] : n){
Ui[j] <- rinvchisq(1, df=DF + 1, scale=(DF*Tau^2 + (obs[j] - mu[m+1])^2/Alpha )/(DF + 1))
...
Tau <- rgamma(1, n*DF/2, scale = DF*sum(1/Ui)/2)
