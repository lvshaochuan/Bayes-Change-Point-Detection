FFBS.Depend <- function(Uniformtimes, JumpProb, eventtimes, obs, param){
n <- length(obs)
...
precision[1] <- 1/sigmaprior^2 + N[1]/Sigma^2
theta[1] <- rnorm(1,  N[1]*mean(obs[cp[1]:(cp[2]-1)])/precision[1], sd=1/precision[1])
for (i in 2:m){
precision[i] <- 1/sigmaprior^2 + N[i]/Sigma^2
theta[i] = rnorm(1, (theta[i-1]/sigmaprior^2 + N[i]*mean(obs[cp[i]:(cp[i+1]-1)])/Sigma^2)/precision[i], sd=1/precision[i])
}
precision[m+1] <- 1/sigmaprior^2 + N[m+1]/Sigma^2
theta[m+1] = rnorm(1, (theta[m]/sigmaprior^2 + N[m+1]*mean(obs[cp[m+1]:n])/Sigma^2)/precision[m+1], sd=1/precision[m+1])
...
return(list(Number=m+1, cp=cp, CP=CP, Q=Q, states= S, theta=theta, filtering=filtering))     
}